...
export class StockService {
  ...
  static async releaseReservation(
    ...
    try {
      ...
      const movementPromises = reservations.map(reservation =>
        prisma.stockMovement.create({
          data: {
  ...
  static async updateStock(
    ...
    reason: 'SALE' | 'RETURN' | 'DAMAGE' | 'THEFT' | 'CORRECTION' | 'RECEIPT' | 'TRANSFER',
    storeId: string,
    userId?: string,
  ...
  static async cleanupExpiredReservations(): Promise<void> {
    try {
      ...
      const movementPromises = expiredReservations.map(reservation =>
        prisma.stockMovement.create({
          data: {
            productId: reservation.productId,
            variantId: reservation.variantId,
   ...
   */
  static async validateMinimumStock(
    productId: string,
    variantId: string | undefined,
